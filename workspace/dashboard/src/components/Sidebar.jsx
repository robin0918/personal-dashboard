...
function Sidebar({ currentPage, onNavigate }) {
  return (
    <aside className="fixed left-0 top-0 h-full w-72 flex flex-col z-50">
      {/* Background with gradient */}
      <div className="absolute inset-0 bg-gradient-to-b from-dark-900/80 to-dark-950/95 backdrop-blur-2xl" />
      ...
      <div className="relative z-10 flex flex-col h-full p-6">
        ...
        >
          <div className="relative">
            <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-primary-400 via-purple-500 to-pink-500 flex items-center justify-center shadow-lg shadow-primary-500/30 animate-pulse-glow">
              <span className="text-2xl">🐉</span>
            </div>
            <div className="absolute -bottom-1 -right-1 w-4 h-4 bg-emerald-500 rounded-full border-2 border-dark-900" />
          </div>
          <div>
            <h1 className="font-bold text-lg gradient-text">龙王</h1>
        ...
        <nav className="flex-1 space-y-2">
          {menuItems.map((item, index) => (
            ...
            >
              <item.icon size={20} className={currentPage === item.id ? 'text-primary-400' : ''} />
        ...
        {/* Pro Banner */}
        <motion.div 
          className="mb-4 p-4 rounded-xl bg-gradient-to-br from-primary-500/20 to-purple-500/20 border border-primary-500/30"
        ...
        >
          ...
          <button className="w-full py-2 rounded-lg bg-gradient-to-r from-primary-500 to-purple-500 text-white text-sm font-medium hover:shadow-lg hover:shadow-primary-500/30 transition-all">
            了解更多
          </button>
        </motion.div>
